...
def generate_adventure_story():
    ...
    story = random.choice(Starter)+" "+random.choice(characters)+" "+random.choice(plot)+" "+random.choice(journey)+" "+random.choice(action)+" "+random.choice(plot_twist)+" "+random.choice(end)
...
def generate_romance_story():
    ...
    story = random.choice(starter)+" "+random.choice(place)+" "+random.choice(bond)+" "+random.choice(connection)+" "+random.choice(moment)+" " +random.choice(more)+" "+random.choice(ending)
    result_text.configure(state=tk.NORMAL)
    result_text.delete('1.0', tk.END)
    result_text.insert(tk.END, story)
    result_text.configure(state=tk.DISABLED)
...
def generate_horror_story():
    ...
    story = random.choice(Starter)+" "+random.choice(Characters)+" "+random.choice(plot)+" "+random.choice(scare)+" "+random.choice(end)
    result_text.configure(state=tk.NORMAL)
    result_text.delete('1.0', tk.END)
    result_text.insert(tk.END, story)
    result_text.configure(state=tk.DISABLED)
...
def generate_mystery_story():
    ...
    story = random.choice(starter)+" "+random.choice(char)+" "+random.choice(plot)+" "+random.choice(clues)+" "+random.choice(end_plot)
    result_text.configure(state=tk.NORMAL)
    result_text.delete('1.0', tk.END)
    result_text.insert(tk.END, story)
    result_text.configure(state=tk.DISABLED)
...
# Genre selection
genre_label = ttk.Label(root, text="Select a genre for your story:",font="Arial 18")
genre_label.grid(row=0, column=0, padx=10, pady=10)

genres = ["Horror", "Romance", "Adventure", "Mystery"]
genre_combobox = ttk.Combobox(root, values=genres,font="Arial 18")
genre_combobox.grid(row=0, column=1, padx=25, pady=30)
genre_combobox.current(0)

# Generate button
generate_button = ttk.Button(root, text="Generate Story", command=generate_story)
generate_button.grid(row=1, column=0, columnspan=2, padx=25, pady=25)

# Result text area
result_text = tk.Text(root, wrap="word", height=50, width=100, state=tk.DISABLED,font="Arial 20")
